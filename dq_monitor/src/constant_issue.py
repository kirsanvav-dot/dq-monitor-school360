...
class IssueType(Enum):
    ...
    тип исправления (всегда tuple CleanType),
    Опционально: порядок внутри пайплайна
    """
   # Completeness — глобально обязательные поля (для всех типов событий)
    EMPTY_EVENT_ID = ('empty_event_id', "Пропущено обязательное значение event_id", DQDimension.COMPLETENESS, ('event_id',), (CleanType.DELETE, CleanType.ZEROING))
    EMPTY_CLIENT_ID = ('empty_client_id', "Пропущено обязательное значение client_id", DQDimension.COMPLETENESS, ('client_id',), (CleanType.DELETE, CleanType.ZEROING))
    EMPTY_EVENT_TYPE = ('empty_event_type', "Пропущено обязательное значение event_type", DQDimension.COMPLETENESS, ('event_type',), (CleanType.DELETE, CleanType.ZEROING))
    EMPTY_EVENT_TS = ('empty_event_ts', "Пропущено обязательное значение event_ts", DQDimension.COMPLETENESS, ('event_ts',), (CleanType.DELETE, CleanType.ZEROING))
    EMPTY_DEVICE_TYPE = ('empty_device_type', "Пропущено обязательное значение device_type", DQDimension.COMPLETENESS, ('device_type',), (CleanType.DELETE, CleanType.ZEROING))
    EMPTY_IP_ADDRESS = ('empty_ip_address', "Пропущено обязательное значение ip_address", DQDimension.COMPLETENESS, ('ip_address',), (CleanType.DELETE, CleanType.ZEROING))
    EMPTY_GEO_COUNTRY = ('empty_geo_country', "Пропущено обязательное значение geo_country", DQDimension.COMPLETENESS, ('geo_country',), (CleanType.DELETE, CleanType.ZEROING))
    EMPTY_GEO_CITY = ('empty_geo_city', "Пропущено обязательное значение geo_city", DQDimension.COMPLETENESS, ('geo_city',), (CleanType.DELETE, CleanType.ZEROING))
    EMPTY_CHANNEL = ('empty_channel', "Пропущено обязательное значение channel", DQDimension.COMPLETENESS, ('channel',), (CleanType.DELETE, CleanType.ZEROING))
    ...
    EMPTY_AMOUNT_RUB = ('empty_amount_rub', "Пропущено обязательное значение amount_rub при типе операции transaction",
                        DQDimension.COMPLETENESS, ('amount_rub',), (CleanType.DELETE, CleanType.ZEROING))
    EMPTY_CURRENCY = ('empty_currency', "Пропущено обязательное значение currency при типе операции transaction",
                      DQDimension.COMPLETENESS, ('currency',), (CleanType.DELETE, CleanType.ZEROING))
    EMPTY_MERCHANT_CATEGORY = ('empty_merchant_category', "Пропущено обязательное значение merchant_category при типе операции transaction",
                               DQDimension.COMPLETENESS, ('merchant_category',), (CleanType.DELETE, CleanType.ZEROING))
    EMPTY_MERCHANT_COUNTRY = ('empty_merchant_country', "Пропущено обязательное значение merchant_country при типе операции transaction",
                              DQDimension.COMPLETENESS, ('merchant_country',), (CleanType.DELETE, CleanType.ZEROING))
    EMPTY_CARD_LAST4 = ('empty_card_last4', "Пропущено обязательное значение card_last4 при типе операции transaction",
                        DQDimension.COMPLETENESS, ('card_last4',), (CleanType.DELETE, CleanType.ZEROING))
    ...
    @property
    def clean_type(self) -> tuple[CleanType]:
        return self.value[4]
    
    @property
    def clean_type_pipeline_order(self) -> int:
        try:
            return self.value[5]
        except IndexError:
            return 0
