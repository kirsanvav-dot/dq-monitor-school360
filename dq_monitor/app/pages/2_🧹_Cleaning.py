...
CLEAN_TYPE_LABELS: dict[CleanType, str] = {
    ...
    CleanType.IGNORE: "Не менять данные",
}
...
<style>
    ...
    /* Таблица правил очистки (Интерфейс) */
    .clean-rules-header { font-weight: 700; font-size: 14px; padding: 6px 4px; border-bottom: 2px solid #cbd5e1; margin-bottom: 4px; color: #1e293b; }
    .clean-rules-row { padding: 6px 4px; border-bottom: 1px solid #e2e8f0; min-height: 2.4rem; white-space: normal; line-height: 1.4; font-size: 14px; color: #334155; }

    /* Selectbox на всю ширину колонки */
    div[data-testid="column"] [data-testid="stSelectbox"] { width: 100% !important; }
    div[data-testid="column"] [data-testid="stSelectbox"] > div { width: 100% !important; }
</style>
...
def run_cached_cleaning(df, enabled_rules_key: tuple[tuple[str, str], ...]):
    enabled_issues = {(IssueType[name], CleanType(ct_value)) for name, ct_value in enabled_rules_key}
...
for dim_enum in DQDimension:
    ...
    with title_col:
        # Названия измерений на английском, как и договорились
        st.markdown(f"#### {dim_enum.value}")
    with btn_col:
        sel_all_btn, sel_none_btn = st.columns(2)
        with sel_all_btn:
            if st.button("Все", key=f"sel_all_{dim_enum.name}", use_container_width=True):
                for issue in issues_in_dim: st.session_state[f"cb_{issue.name}"] = True
        with sel_none_btn:
            if st.button("Снять", key=f"sel_none_{dim_enum.name}", use_container_width=True):
                for issue in issues_in_dim: st.session_state[f"cb_{issue.name}"] = False
    ...
    for issue in issues_in_dim:
        ...
        with row[0]:
            enabled = st.checkbox("вкл", key=f"cb_{issue.name}", label_visibility="collapsed")
        with row[1]:
            st.markdown(f'<div class="clean-rules-row">{issue.description}</div>', unsafe_allow_html=True)
        with row[2]:
            selected_label = st.selectbox(
                "Способ", options=labels, index=labels.index(default_label),
                key=f"action_{issue.name}", label_visibility="collapsed", disabled=not enabled,
            )
...
if st.button("🚀 Запустить очистку выбранных пунктов"):
    if not enabled_rules:
        ...
        cache_key = tuple(sorted((issue.name, ct.value) for issue, ct in enabled_rules))
...
if "cleaning_log" in st.session_state:
    ...
    st.download_button("💾 Скачать очищенный датасет (CSV)", data=csv, file_name="cleaned_events.csv", mime="text/csv")