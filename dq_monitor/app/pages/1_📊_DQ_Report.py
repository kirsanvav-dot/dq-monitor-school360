...
report = Report(
    ...
    issues=[
        ...
        DQIssue(issue_type=IssueType.EMPTY_EVENT_ID, affected_indices=pd.Index([1]), rows_affected=1),
        DQIssue(issue_type=IssueType.EMPTY_CLIENT_ID, affected_indices=pd.Index([2, 3]), rows_affected=2),
        DQIssue(issue_type=IssueType.EMPTY_EVENT_TS, affected_indices=pd.Index([4]), rows_affected=1),
        DQIssue(issue_type=IssueType.EMPTY_DEVICE_TYPE, affected_indices=pd.Index([5, 6, 7]), rows_affected=3),
        DQIssue(issue_type=IssueType.EMPTY_GEO_CITY, affected_indices=pd.Index([8, 9]), rows_affected=2),
        DQIssue(issue_type=IssueType.EMPTY_AMOUNT_RUB, affected_indices=pd.Index([10]), rows_affected=1),
        DQIssue(issue_type=IssueType.EMPTY_CURRENCY, affected_indices=pd.Index([11, 12]), rows_affected=2),
        DQIssue(issue_type=IssueType.EMPTY_FLAG_REASON, affected_indices=pd.Index([13]), rows_affected=1),

        # VALIDITY (Некорректный формат)
        DQIssue(issue_type=IssueType.INVALID_FORMAT_DATE, affected_indices=pd.Index([14, 15]), rows_affected=2),
        DQIssue(issue_type=IssueType.INVALID_IP_ADDRESS, affected_indices=pd.Index([16]), rows_affected=1),
        DQIssue(issue_type=IssueType.INVALID_AMOUNT_RUB, affected_indices=pd.Index([17, 18, 19]), rows_affected=3),
        DQIssue(issue_type=IssueType.INVALID_CURRENCY, affected_indices=pd.Index([20, 21]), rows_affected=2),
        DQIssue(issue_type=IssueType.INVALID_MERCHANT_CATEGORY, affected_indices=pd.Index([22]), rows_affected=1),
        DQIssue(issue_type=IssueType.INVALID_CARD_LAST4, affected_indices=pd.Index([23, 24]), rows_affected=2),
        DQIssue(issue_type=IssueType.INVALID_DEVICE_TYPE, affected_indices=pd.Index([25]), rows_affected=1),

        # CONSISTENCY (Противоречивость)
        DQIssue(issue_type=IssueType.INCONSISTENCY_FLAGGED, affected_indices=pd.Index([26, 27]), rows_affected=2),
        DQIssue(issue_type=IssueType.INCONSISTENCY_TRANSACTION, affected_indices=pd.Index([28]), rows_affected=1),
        DQIssue(issue_type=IssueType.INCONSISTENCY_SESSION, affected_indices=pd.Index([29, 30, 31]), rows_affected=3),

        # UNIQUENESS (Дубликаты)
        DQIssue(issue_type=IssueType.DUPLICATE_FULL, affected_indices=pd.Index([32, 33]), rows_affected=2),
        DQIssue(issue_type=IssueType.DUPLICATE_EVENT_ID, affected_indices=pd.Index([34, 35, 36]), rows_affected=3)
    ]
)
