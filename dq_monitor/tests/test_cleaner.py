...
def test_funny_country_corrects(medium_clean_df: pd.DataFrame):
    ...
    df, index = cleaner.DataCleaner._clean_invalid_geo_country_zeroing(cleaner.DataCleaner(), df)
...
def test_consistency_ts_dont_breaks(medium_clean_df: pd.DataFrame):
    ...
    df, index = cleaner.DataCleaner._clean_inconsistency_session_timestamps_correction(cleaner.DataCleaner(), df)
    assert len(index) == 0
