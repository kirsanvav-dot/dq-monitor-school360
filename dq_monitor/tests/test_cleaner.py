...
def test_funny_country_corrects(medium_clean_df):
...
def test_consistency_flags_right_rows(medium_clean_df):
    df = medium_clean_df.copy()
    
    df, index = cleaner.DataCleaner._clean_inconsistency_flagged_field(cleaner.DataCleaner(), df)
    assert len(index) == 0

def test_consistency_ts_dont_breaks(medium_clean_df):
    df = medium_clean_df.copy()
